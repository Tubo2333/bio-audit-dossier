...
Variant (error-injection switch, injected by make_variants_10x.py):
    A — textbook 10X pipeline with scDblFinder doublet detection (positive control, no injected error)
...
- Doublet detection: scDblFinder, random-forest doublet scoring on simulated doublets (Xi & Li 2021, Genome Biology, PMID 33541407); 10X/droplet data are subject to 0.5-8% doublet rates, and unidentified doublets create spurious co-expression and false 'new cell types'
...
# ---------------------------------------------------------------------------
# Variant: A  (injected by make_variants_10x.py — error switch)
# ---------------------------------------------------------------------------
VARIANT = "A"
...
def step_doublet(adata, work_dir, rscript):
    """scDblFinder doublet detection (Xi & Li 2021, PMID 33541407) —
    Bioconductor reference implementation via Rscript subprocess (the
    current best-practice method for droplet/10X data; random-forest
    classifier on simulated doublets)."""
    import scipy.io
    # export QC-filtered UMI counts as genes x cells (scDblFinder input
    # orientation; smoke-verified) in MatrixMarket format; drop genes with
    # zero counts across the retained cells (documented input precondition
    # of the reference implementation)
    counts = adata.X.T.tocsr().astype(np.int32)
    gs = np.asarray(counts.sum(axis=1)).ravel()
    counts = counts[gs > 0]
    mtx_path = work_dir / "dbl_counts.mtx"
    scipy.io.mmwrite(mtx_path, counts)
    # per-cell sample-of-origin (33 pooled 10X libraries): doublet rates are
    # library-specific — the reference implementation computes per-sample
    # doublet fractions when `samples` is provided
    pd.Series(adata.obs["sample"].astype(str).values, name="sample").to_csv(
        work_dir / "dbl_samples.csv", index=False, header=False)
    r_code = r"""
library(Matrix)
library(scDblFinder)
counts <- readMM("dbl_counts.mtx")
counts <- as(counts, "dgCMatrix")
samples <- read.csv("dbl_samples.csv", header=FALSE)$V1
cat("DBL_IN dim=", dim(counts)[1], "x", dim(counts)[2], " nnz=", length(counts@x),
    " n_samples=", length(unique(samples)), "\n")
set.seed(42)
dbl <- scDblFinder::scDblFinder(counts, samples = samples)
cls <- colData(dbl)$scDblFinder.class
sc <- colData(dbl)$scDblFinder.score
write.csv(data.frame(idx = seq_len(length(cls)), class = as.character(cls),
                     score = round(as.numeric(sc), 6)),
          "dbl_results.csv", row.names = FALSE)
cat("DBL_DONE n=", length(cls), " doublets=", sum(cls == "doublet"), "\n")
"""
    r_file = work_dir / "dbl_scDblFinder.R"
    ...
    if proc.returncode != 0:
        raise RuntimeError("scDblFinder failed:\n" + proc.stderr[-3000:])
    res = pd.read_csv(work_dir / "dbl_results.csv")
    assert len(res) == adata.n_obs, (
        f"scDblFinder result length {len(res)} != n_obs {adata.n_obs}")
    dbl_mask = res["class"].values == "doublet"
    n_doublets = int(dbl_mask.sum())
    rate = round(n_doublets / adata.n_obs, 4)
    print(f"[doublet] scDblFinder: {n_doublets}/{adata.n_obs} doublets "
          f"({rate:.2%}) removed", flush=True)
    adata = adata[~dbl_mask].copy()
...
def main() -> int:
    ...
    resp = report_decision(
        ...
        "scDblFinder",
        "Doublet detection rationale: scDblFinder random-forest doublet detection on simulated doublets — the best-performing doublet-detection method in benchmarks for droplet/10X data (Xi & Li 2021, PMID 33541407); executed with the Bioconductor reference implementation.",
        {"n_cells": n_cells_after_qc, "n_doublets": n_doublets, "n_doublet_rate": n_doublet_rate, "n_patients": n_patients, "has_batch": n_patients > 1},
        "scDblFinder::scDblFinder (Rscript subprocess)",
        "scDblFinder::scDblFinder(counts, seed=42); remove class==doublet cells",
    )
