...
Variant (error-injection switch, injected by make_variants.py):
    C — hard-threshold QC replaces MAD-adaptive QC (fixed cutoffs)
...
# ---------------------------------------------------------------------------
# Variant: C  (injected by make_variants.py — error switch)
# ---------------------------------------------------------------------------
VARIANT = "C"
...
def main() -> int:
    ...
    resp = report_decision(
        reporter, "qc_filtering", "qc_filtering", "hard_threshold",
        "QC filtering rationale: Fixed hard thresholds (min_genes=200, min_cells=3) applied globally.",
        {"min_genes": 200, "min_cells": 3, "n_cells_before": n_cells_before_qc, "n_cells_after": n_cells_after_qc, "n_patients": n_patients, "has_batch": n_patients > 1},
