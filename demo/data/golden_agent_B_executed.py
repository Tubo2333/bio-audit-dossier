...
Variant (error-injection switch, injected by make_variants.py):
    B — cell-level Wilcoxon DEG replaces patient-level pseudobulk (pseudoreplication)
...
- Differential expression: cell-level Wilcoxon rank-sum test between two annotated cell types (scanpy default; Squair et al. 2021, Nat Commun, PMID 34433851)
...
# ---------------------------------------------------------------------------
# Variant: B  (injected by make_variants.py — error switch)
# ---------------------------------------------------------------------------
VARIANT = "B"
...
def main() -> int:
    ...
    # Step 10: differential expression
        # cell-level Wilcoxon rank-sum test on all cells
    # (each cell treated as an independent sample)
    tic("deg")
    abund = adata.obs["cell_type"].value_counts()
    top2 = abund.index[:2].tolist()
    sub = adata[adata.obs["cell_type"].isin(top2)].copy()
    sc.tl.rank_genes_groups(sub, groupby="cell_type", method="wilcoxon",
                            use_raw=False)
    names = pd.DataFrame(sub.uns["rank_genes_groups"]["names"])
    pvals = pd.DataFrame(sub.uns["rank_genes_groups"]["pvals"])
    lfc = pd.DataFrame(sub.uns["rank_genes_groups"]["logfoldchanges"])
    res = pd.DataFrame({
        "gene": names[top2[0]].values,
        "pvalue": pvals[top2[0]].values,
        "logFC": lfc[top2[0]].values,
    })
    resp = report_decision(
        reporter, "deg_method", "deg_method", "wilcoxon_rank_sum",
        "Wilcoxon rank-sum test between two annotated cell types at the "
        "single-cell level (scanpy default).",
        {"design": "simple_two_group", "unit": "cell",
         "deg_test": "wilcoxon", "n_patients": n_patients},
